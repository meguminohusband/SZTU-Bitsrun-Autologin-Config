...
| `user_type` | 运营商（`cucc`/`ctcc`/空） | ✅ 按实际情况填 |
| `password` | 校园网密码 | ✅ 必填 |
...
| `n` | 认证参数 | 保持默认 `200` |
| `type` | 认证类型 | 保持默认 `1` |
| `acid` | 认证服务器标识 | 已知北区宿舍为 `17`，南区或公共区域请自行抓包 |
...
本仓库的配置模板与文档采用 [MIT 许可证](./LICENSE)。

登录软件 BitSrunLoginGo 本身的许可证以原作者项目为准（AGPL-3.0）。
